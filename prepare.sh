...
# fix UnicodeEncodeError when saving Korean text to jsonl.gz
...
dl_dir=$PWD/download
zeroth_dir=$dl_dir/zeroth_korean
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
    log "Stage 1: Prepare KsponSpeech and Zeroth-Korean manifests"
    ...
    if [ -d $zeroth_dir ] && [ ! -e $data/manifests/.zeroth.done ]; then
        ./local/prepare_zeroth_korean.py \
            --zeroth-dir $zeroth_dir \
            --output-dir $data/manifests
        touch $data/manifests/.zeroth.done
    fi
fi
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
    log "Stage 3: Compute fbank for KsponSpeech and Zeroth-Korean"
    mkdir -p $data/fbank
    ...
    if [ -d $zeroth_dir ] && [ ! -e $data/fbank/.zeroth.done ]; then
        ./local/compute_fbank_zeroth.py --data-dir $data
        touch $data/fbank/.zeroth.done
    fi
    if [ -d $zeroth_dir ] && [ ! -e $data/fbank/.zeroth-validated.done ]; then
        log "Validating data/fbank for Zeroth-Korean"
        parts=(
            train
            test
        )
        for part in ${parts[@]}; do
            ./local/validate_manifest.py \
                $data/fbank/zeroth_cuts_${part}.jsonl.gz
        done
        touch $data/fbank/.zeroth-validated.done
    fi
fi
...
if [ $stage -le 5 ] && [ $stop_stage -ge 5 ]; then
    ...
    for vocab_size in ${vocab_sizes[@]}; do
        ...
        if [ ! -f $lang_dir/transcript_words.txt ]; then
            ...
            if [ -d $zeroth_dir ]; then
                files=$(
                    echo ${files} $(find "$data/fbank" -name "zeroth_cuts_*.jsonl.gz")
                )
            fi
            ./local/prepare_transcript_words.py \
                --cuts ${files} \
